...
function strengthenBulletFallback(field, value) {
  ...
  if (!DOMAIN_RE.test(text)) additions.push('for cybersecurity SaaS adoption, retention, and customer-success execution');
  if (!MECHANISM_RE.test(text) || hasWeakGenericShape(text)) {
    additions.push('by aligning stakeholders, use cases, and measurable renewal outcomes');
  }
  ...
  if (targets && wordCount(text) < targets.minWords) {
    text = `${text}, with executive-ready operating rhythm and clear account-health ownership`;
  }
  if (targets && text.length < targets.minChars) {
    text = `${text}, tying daily execution to durable adoption, risk reduction, and retention signals`;
  }

  return `${text}.`;
}

async function repairQualityIssues(replacements, fields, issues, log, notify = () => {}) {
  const failedFields = qualityIssueFields(issues)
    .filter(field => /^JOB_\d+_BULLET_\d+$/i.test(field) && fields.includes(field));
  if (!failedFields.length) return replacements;

  let repaired = { ...replacements };
  for (const field of failedFields) {
    repaired[field] = strengthenBulletFallback(field, repaired[field]);
  }
  ...
  try {
    ...
    const prompt = `Repair only the listed resume bullets so they pass the quality gate while preserving only facts already present in each bullet. Keep each bullet concise and impact-oriented. Every bullet must include enterprise/stakeholder proof, customer-success or security/SaaS domain language, and a concrete mechanism. Do not invent employers, dates, certifications, product names, or exact metrics. Return strict JSON mapping field names to repaired bullets only.\n\nFAILED ISSUES:\n${issues.join('\n')}\n\nBULLETS:\n${JSON.stringify(Object.fromEntries(failedFields.map(field => [field, repaired[field]])))}`;
...
export async function generateResumeFinish(state) {
  ...
  try {
    ...
    replacements = await repairQualityIssues(replacements, fields, err.issues ?? [], log, emit);
    try {
      validateResumeQuality(replacements, fields);
