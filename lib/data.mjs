...
const CONFIG_DIR = process.env.CAREER_OPS_CONFIG_DIR
  ? path.resolve(process.env.CAREER_OPS_CONFIG_DIR)
...
export function loadBragDoc() {
  const filePath = path.resolve(DATA_DIR, 'master-brag-document.md');
...
export function loadProfile() {
  const filePath = path.resolve(CONFIG_DIR, 'profile.yml');
...
export function loadResumeText() {
  const filePath = path.resolve(DATA_DIR, 'Brian_Milhizer_Resume.docx');
...
export function loadLinkedInText() {
  const filePath = path.resolve(DATA_DIR, 'Profile.pdf');
  if (!fs.existsSync(filePath)) {
    _linkedInCache = null;
...
export function loadPipeline() {
  const filePath = path.resolve(DATA_DIR, 'pipeline.md');
...
export function updateJob(jobId, fields) {
  if (typeof fields === 'function') return updateStoredJob(jobId, fields);
